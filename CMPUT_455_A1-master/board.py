...
class GoBoard(object):
    def __init__(self, size: int) -> None:
        ...
        self.move_history = []  # Stores the history of moves and their consequences
    ...
    def play_move(self, point: GO_POINT, color: GO_COLOR) -> bool:
        ...
        if self.board[point] != EMPTY:
            return False
        previous_state = (self.board.copy(), self.black_captures, self.white_captures)
        ...
        for offset in offsets:
            if self.board[point+offset] == O and self.board[point+(offset*2)] == O and self.board[point+(offset*3)] == color:
                ...
                if color == BLACK:
                    ...
                    self.white_captures += 2
        self.move_history.append((point, previous_state, captured_points))            
        return True
    ...
    def undo_move(self) -> None:
        ...
        """
        last_move, previous_state, captured_points = self.move_history.pop()
        self.board, self.black_captures, self.white_captures = previous_state
        
        # Restoring the captured stones
        for point in captured_points:
            self.board[point] = opponent(self.board[last_move])
    
    def detect_winner(self) -> GO_COLOR:
