...
class GtpConnection:
    ...
    def genmove_cmd(self, args: List[str]) -> None:
        ...
        _, move = self.alphabeta(color)
    
        # Check if the minimax returns a valid move
        if move is not None:
            TheMove = format_point(point_to_coord(move, self.board.size)).lower()
            self.respond(TheMove)
        else:
            self.respond("pass")  # The current player passes if no move is returned
    ...
    def solve_cmd(self, args: List[str]) -> None:
        color = self.board.current_player
        score, move = self.alphabeta(color)
        TheMove = format_point(point_to_coord(move, self.board.size)).lower() if move is not None else "unknown"

        if score is None:
            self.respond("unknown")
        elif score == 999:
            if color == 1:
               self.respond(f"b {TheMove}")  # Black wins
            else:
               self.respond("b")
        elif score == -999:
            if color == 2:
               self.respond(f"w {TheMove}")  # White wins
            else:
               self.respond("w")
        elif score == 0:
            self.respond(f"draw {TheMove}")  # Draw
        else:
            self.respond("unknown")  # Time limit exceeded or other unknown result


    def alphabeta(self, color, depth=3, alpha=-float('inf'), beta=float('inf')) -> Tuple[int, GO_POINT]:
        bestMove = None
        self.start_time = time.time()  # Store the start time

        def time_exceeded() -> bool:
            return time.time() - self.start_time > self.go_engine.time_limit

        if time_exceeded() or depth == 0 or self.board.get_empty_points().size == 0 or self.board.detect_winner() != 0:
            winner = self.board.detect_winner()
            return (999 if winner == 1 else -999 if winner == 2 else 0), bestMove
        
        if color == 1:  # balck /max
            maxEval = -float('inf')
            for move in self.board.get_empty_points():
                ...
                if self.board.is_legal(move, color):
                    ...
                    eval, _ = self.alphabeta(2, depth-1, alpha, beta)
            ...
            for move in self.board.get_empty_points():
                ...
                if self.board.is_legal(move, color):
                    self.board.play_move(move, color)
                    eval, _ = self.alphabeta(1, depth-1, alpha, beta)
                    self.board.undo_move()
                    if eval < minEval:
