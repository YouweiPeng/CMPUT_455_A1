...
class GtpConnection:
    ...
    def solve_cmd(self, args: List[str]) -> None:
        ...
        winner, move = self.minimax(color)
        TheMove=format_point(point_to_coord(move, self.board.size)).lower()
        if winner == 1:
            self.respond("b")  # Black wins
        elif winner == -1:
            self.respond("w")  # White wins
        else:
            self.respond(f"draw {TheMove}")  # Draw
        return
        # self.respond(self.board.detect_winner())


    def minimax(self, color) -> Tuple[int, GO_POINT]:
        bestMove=None
        if color == 1:
            bestscore = -inf
        else:
            bestscore = inf
        if self.board.get_empty_points().size == 0 or self.board.detect_winner() != 0:
            winner = self.board.detect_winner()
            if winner == 1:
                return 1, bestMove  # Black wins
            elif winner == 2:
                return -1, bestMove  # White wins
            else:
        ...
        for move in self.board.get_empty_points():
            ...
            if color == 1:
                ...
                if self.board.is_legal(move, color):
                    ...
                    copy.play_move(move, color)
                    if copy.black_captures > self.board.black_captures:
                        bestscore = 0.5
                        bestMove = move
                        continue
                    self.board.play_move(move, color)
                    score, _ = self.minimax(opponent(color))
                    if score > bestscore:
                        bestscore = score
                        bestMove = move
            if color == 2:
                ...
                if self.board.is_legal(move, color):
                    copy=self.board.copy()
                    copy.play_move(move, color)
                    if copy.white_captures > self.board.white_captures:
                        bestscore = -0.5
                        bestMove = move
                        continue
                    self.board.play_move(move, color)
            ...
            print(self.board2d())
            if bestMove!=None:
                Check=format_point(point_to_coord(bestMove,self.board.size)).lower()
            else:
                Check="not yet"
            print(f"I am {color}, my best socre is {bestscore}, my best move is {Check}")
            print(f"w:{self.board.white_captures}, b:{self.board.black_captures}")
